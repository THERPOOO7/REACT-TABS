...
const Emplisting = () => {
  ...
  const Removefunction = (id) => {
    if (window.confirm('Do you want to Delete ?')) {
      fetch("http://localhost:5555/usrs/" + id, {
        method: 'DELETE',
      })
        .then((res) => {
  ...
  useEffect(() => {
    fetch("http://localhost:5555/usrs").then((res) => {
      return res.json();
    }).then((resp) => {
      empdatachange(resp);
    }).catch((err) => {
              ...
              <table className="table">
                ...
                <tbody>
                    ...
                    empdata.map(item => (
                      <tr key={item.id}>
                        ...
                        <td style={{ 'display': 'flex', 'justify-content': 'space-evenly' }}>
                          ...
                          <button onClick={() => { Removefunction(item.id) }} className='btn btn-danger mx-2'>Remove</button>
                  ...
                  <div className='back_btn'>
                    <Link className='btn btn-success' style={{ color: 'green', }} to='/data/create'> Back</Link>
                  </div>
                </tbody>
              </table>
            </div>
