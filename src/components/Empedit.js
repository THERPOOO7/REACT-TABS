...
const Empedit = () => {
  ...
  const [id, setId] = useState('')
    ...
    const handleSubmit = (e) => {
        ...
        fetch("http://localhost:5555/usrs/" +daid, {
            method: 'PUT',
  ...
  useEffect(() => {
    fetch("http://localhost:5555/usrs/" +daid).then((res) => {
      return res.json();
    }).then((resp) => {
      setId(resp.id)
      setName(resp.name)
      ...
      setEventDate(resp.eventDate)
      setEventTime(resp.eventTime)
      setEventDescription(resp.eventDescription)
    }).catch((err) => {
  ...
  return (
    <div>
      ...
      <div >
        <h1 className='mt-5' style={{ textAlign: 'center' }}>Edit detail</h1>
          ...
          <div className="border rounded  p-4">
            <form onSubmit={handleSubmit}>
              ...
              <div>
                ...
                <Link className="btn btn-success mt-3" to='/' > Go back</Link>
              </div>
            </form>
            <div>
            </div>
